...
exports.recalculateDifference = exports.calculateMonthlySales = void 0;
const typegoose_1 = require("@typegoose/typegoose");
const PaymentModel_1 = require("./PaymentModel"); // Adjust the path as needed
...
//   }
// }
async function calculateMonthlySales(isRecent) {
    try {
        const pipelineForRandomSubset = [];
        const pipelineForAllRecords = [];
        if (isRecent) {
            const now = new Date();
            const startOfPreviousMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
        ...
        for (const stat of randomSubsetStats) {
            ...
            // }
            // Update or create record in MonthlySales
            await MonthlySalesModel_1.MonthlySalesModel.findOneAndUpdate({ year, month }, {
                year,
                month,
                uniqueCount,
                total,
...
exports.calculateMonthlySales = calculateMonthlySales;
async function recalculateDifference(isCurrentMonthOnly) {
    try {
        let monthsToProcess = [];
        if (isCurrentMonthOnly) {
            ...
            const allRecords = await MonthlySalesModel_1.MonthlySalesModel.find({});
            ...
            monthsToProcess = Array.from(uniqueYearMonths).map(key => {
                const [year, month] = key.split('-').map(Number); // Asserting that key is a string
                return { year, month };
            });
        }
        ...
        for (const monthRecord of monthsToProcess) {
            const salesRecord = await MonthlySalesModel_1.MonthlySalesModel.findOne({ year: monthRecord.year, month: monthRecord.month });
            ...
            if (salesRecord) {
                ...
                const previousMonthRecord = await MonthlySalesModel_1.MonthlySalesModel.findOne({ year: yearOfPreviousMonth, month: previousMonth });
...
exports.recalculateDifference = recalculateDifference;
