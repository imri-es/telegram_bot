...
const MonthlySalesModel_1 = require("./MonthlySalesModel");
const Calculate_1 = require("./Calculate");
const node_cron_1 = __importDefault(require("node-cron"));
(0, db_1.connectDB)();
const bot = new telegraf_1.Telegraf('6793590260:AAEkhRXQP1F0S388gG_5QNdSLEVG3O8lwRY');
...
bot.command('s', async (ctx) => {
    try {
        await (0, Calculate_1.calculateMonthlySales)(true);
        await (0, Calculate_1.recalculateDifference)(true);
        // Fetch data from DB and sort by year and month
        const salesData = await MonthlySalesModel_1.MonthlySalesModel.find().sort({ year: 1, month: 1 });
        const MonthNames = [
...
bot.command('cur', async (ctx) => {
    try {
        (0, Calculate_1.calculateMonthlySales)(true);
        (0, Calculate_1.recalculateDifference)(true);
...
bot.command('all', async (ctx) => {
    try {
        (0, Calculate_1.calculateMonthlySales)(false);
        (0, Calculate_1.recalculateDifference)(false);
